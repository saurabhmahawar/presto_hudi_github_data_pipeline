...
| **Commit Timeline (`.hoodie/`)** | An immutable transaction log tracking all commits, cleans, compactions, and rollbacks. | Stored alongside Parquet files in MinIO. Powers **snapshot isolation** for Presto queries, ensuring readers only scan fully committed data without locking. |
...
* **Detailed Task Breakdown:**
  ...
  2. **`trigger_hudi_ingestion` (Task 2):**
     ...
     * **Circuit Breaker:** Captures the full Spark log once the job exits. If Hudi reports `"Nothing to commit"`, the task raises an explicit `RuntimeError` to prevent false-positive pipeline successes when zero new records were ingested.
...
* **Demonstrations Implemented:**
  ...
  3. **Commit Timeline Inspection:** Queries the Hudi commit timeline (`_hoodie_commit_time`) to list the distinct commit instants recorded in the timeline.
...
2. Toggle the `github_events_ingestion` DAG to **Active** (unpause). Unpausing automatically schedules and starts the first ingestion run.
3. To execute additional batches on-demand, click **▶ Trigger DAG** (under the Actions column).

*(CLI alternative: run `docker exec airflow airflow dags unpause github_events_ingestion` to start; use `docker exec airflow airflow dags trigger github_events_ingestion` only for subsequent manual runs)*
...
> **Runtime note:** A full 24-hour batch processes ~1.3–1.8 million GitHub events across 16 event-type partitions. Downloading raw archives from GitHub dominates the runtime.
...
3. **Commit Timeline Inspection:** Reads Hudi's `.hoodie/` timeline to list the commit instants recorded for the table.
...
The analytical presentation layer is powered by **[Apache Superset](https://superset.apache.org/)**, delivering interactive business intelligence directly on top of the Apache Hudi lakehouse via distributed **PrestoDB** queries.
...
Tracks enterprise engagement, event audits, community star velocity, project branching, and pull request acceptance rates.
...
* **Recent Events (Table):** Most recent events ordered by `event_time` (`event_time`, `type`, `actor_login`), enabling end-to-end verification of the latest ingested batch.
...
| **9** | **Recent Events** | Table (Raw Records) | Last 20 raw events (`DESC`) | Event stream verification and auditing for the latest batch. |
...
* **Production Reality:** In real-world enterprise deployments, event data is almost always partitioned by **ingestion date** (`year=.../month=.../day=...`). GitHub events are heavily skewed — `PushEvent` typically accounts for 65–90% of events depending on the window, creating uneven partition sizes. Date partitioning guarantees evenly distributed file sizes across object storage.
